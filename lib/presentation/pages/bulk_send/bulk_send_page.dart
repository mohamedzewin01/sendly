...
class _BulkSendPageState extends State<BulkSendPage>
  ...
  bool _selectAll = false;
  String _filterBy = 'all'; // all, valid, invalid
  bool _isSending = false; // حالة الإرسال
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                'إرسال رسالة واحدة لجهات اتصال متعددة',
                style: TextStyle(
                  color: Colors.grey[600],
                  fontSize: isMobile ? 14 : 16,
                ),
  ...
  Widget _buildMessageInput(bool isMobile) {
    return TextFormField(
      ...
      maxLines: isMobile ? 4 : 5,
      maxLength: AppConstants.maxMessageLength,
      enabled: !_isSending, // تعطيل عند الإرسال
      validator: (value) {
        if (value == null || value.isEmpty) {
          return AppStrings.fieldRequired;
        }
      ...
      decoration: InputDecoration(
        ...
        hintText: 'اكتب الرسالة التي تريد إرسالها لجميع جهات الاتصال المحددة',
        prefixIcon: const Icon(Icons.message, color: AppConstants.whatsappGreen),
        suffixIcon: _messageController.text.isNotEmpty && !_isSending
            ? IconButton(
          icon: const Icon(Icons.clear),
          onPressed: () => _messageController.clear(),
        )
        ...
        filled: true,
        fillColor: _isSending ? Colors.grey[100] : Colors.grey[50],
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(AppConstants.defaultBorderRadius),
          borderSide: const BorderSide(
            color: AppConstants.whatsappGreen,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'سيتم إرسال رسالة واحدة إلى ${_selectedContacts.length} جهة اتصال',
                  style: TextStyle(
                    color: AppConstants.whatsappDarkGreen,
                    fontWeight: FontWeight.w600,
                    fontSize: isMobile ? 14 : 16,
                ...
                const SizedBox(height: 4),
                Text(
                  'سيتم فتح واتساب مرة واحدة مع جميع الأرقام المحددة',
                  style: TextStyle(
                    color: AppConstants.whatsappDarkGreen.withOpacity(0.7),
                    fontSize: isMobile ? 12 : 14,
                  ),
                ),
              ],
      ...
      children: [
        Expanded(
          ...
          child: SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                _isSending ? 'جاري الإرسال...' : 'إرسال للمحددين',
                style: TextStyle(
                  fontSize: isMobile ? 14 : 16,
                  fontWeight: FontWeight.bold,
                ),
        ...
        Expanded(
          ...
          child: SizedBox(
            height: 50,
            child: OutlinedButton.icon(
              onPressed: widget.contacts.isNotEmpty && !_isSending ? _toggleSelectAll : null,
              icon: Icon(
                _selectAll ? Icons.clear_all : Icons.select_all,
                size: 20,
              ),
  ...
  Widget _buildFilterChip(String filterKey, String label, int count) {
    ...
    return FilterChip(
      label: Text('$label ($count)'),
      selected: isActive,
      onSelected: _isSending ? null : (_) => _changeFilter(filterKey),
      selectedColor: AppConstants.whatsappGreen.withOpacity(0.2),
      checkmarkColor: AppConstants.whatsappGreen,
      labelStyle: TextStyle(
        color: isActive ? AppConstants.whatsappGreen : Colors.grey[700],
  ...
  Widget _buildContactItem(Contact contact) {
    ...
    return Container(
      ...
      child: CheckboxListTile(
        value: isSelected,
        onChanged: _isSending ? null : (bool? value) => _toggleContactSelection(contact),
        title: Text(
          contact.name,
          style: TextStyle(
            fontWeight: FontWeight.bold,
  ...
  void _handleBulkSend() async {
    ...
    if (invalidContacts.isNotEmpty) {
      final confirmed = await AppUtils.showConfirmDialog(
        context,
        'أرقام غير صحيحة',
        'يوجد ${invalidContacts.length} جهة اتصال بأرقام غير صحيحة. سيتم إرسال الرسالة للأرقام الصحيحة فقط (${validContacts.length} رقم).',
        confirmText: 'متابعة',
        icon: Icons.warning,
      );
  ...
  void _performBulkSend(List<Contact> contacts) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Row(
          children: [
            Icon(Icons.send, color: AppConstants.whatsappGreen),
            SizedBox(width: 8),
            Text('تأكيد الإرسال'),
          ],
        ),
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'سيتم إرسال رسالة واحدة إلى ${contacts.length} جهة اتصال',
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 8),
                  const Text(
                    'سيتم فتح واتساب مع جميع الأرقام المحددة في محادثة واحدة',
            ...
            const SizedBox(height: 16),
            const Text('جهات الاتصال المحددة:'),
            const SizedBox(height: 8),
            Container(
              height: 120,
              decoration: BoxDecoration(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            onPressed: _isSending ? null : () {
              Navigator.pop(context);
              _executeSingleBulkSend(contacts);
            },
            icon: _isSending
                ? const SizedBox(
              width: 16,
  ...
  /// تنفيذ الإرسال الجماعي كرسالة واحدة
  void _executeSingleBulkSend(List<Contact> contacts) async {
    setState(() {
      _isSending = true;
    });
    ...
    try {
      ...
      AppUtils.showCustomSnackBar(
        context,
        'تم إرسال الرسالة بنجاح إلى ${contacts.length} جهة اتصال',
        isSuccess: true,
      );
